    ...
    return (
        <div className="max-w-4xl mx-auto px-6 py-12 md:py-24">
            <ScrollReveal className="text-center mb-16">
                <h1 className="text-4xl md:text-5xl font-display font-bold mb-4 text-transparent bg-clip-text bg-gradient-to-r from-white to-white/60">Press & Media</h1>
                    ...
                    <a key={item.id} href={item.link} className="block group">
                        <GlassCard className="flex flex-col md:flex-row items-baseline md:items-center justify-between !p-6 hover:bg-white/10 transition-all border-white/5 hover:border-indigo-400/30">
                            <div>
                                ...
                                <h3 className="text-xl font-display font-semibold text-white group-hover:text-indigo-300 transition-colors">{item.title}</h3>
                            </div>
                            <div className="text-sm text-slate-400 mt-4 md:mt-0 font-medium font-mono">{item.date}</div>
                        </GlassCard>
                    </a>
                ))}
            </div>
